...
import { EmailNotConfirmedError, UserIsOver13Error, UserWithIdNotFoundError, UserWithUsernameIsExistError } from "@libs/core/custom-error";
...
export class UpdateUserUseCase {
    ...
    private async doOperation(
        ...
        try {
            ...
            if (!user) return Result.Err(new UserWithIdNotFoundError(data.userId))
            if (!user.emailConfirmed) return Result.Err(new EmailNotConfirmedError())

            const isOver13 = userIsOver13(dateOfBirth)
            if (!isOver13) return Result.Err(new UserIsOver13Error())

            const isUsername = await this.userRepo.getUserByUsername(username)
            if (isUsername && user.username !== username) return Result.Err(new UserWithUsernameIsExistError(username))
            ...
            console.log(e)
            return Result.Err(`${UpdateUserUseCase.name} some error`)
        }
    }
}