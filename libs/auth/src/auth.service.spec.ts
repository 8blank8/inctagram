...
import { startTestConfig } from 'apps/inctagram/test/utils/start.app';
// import { createdUsersConfirmationCode } from './created.users.confirmation.code';
import { getTestUser } from 'apps/inctagram/test/utils/get.test.user';
...
describe('auth', () => {
  ...
  let createdUsersConfirmationCode = {}
  ...
  describe('POST /auth/confirmation-code', () => {
    ...
    it('should be status 204 confirmed', async () => {
      await request(app.getHttpServer())
        .post(`/auth/confirmation-code`)
        .send({ code: createdUsersConfirmationCode[user1.email], email: user1.email })
        .expect(HttpStatus.NO_CONTENT)
    })
  ...
  describe('POST /auth/resend-confirmation-code', () => {
    ...
    it('should be status 204 confirmed', async () => {
      await request(app.getHttpServer())
        .post(`/auth/confirmation-code`)
        .send({ code: createdUsersConfirmationCode[user2.email], email: user2.email })
        .expect(HttpStatus.NO_CONTENT)
    })
  })
